...
| `Attempt` | номер/количество текущих попыток |
...
Точные Go-типы, nullable-семантика, формат `Payload`, `Result` и `Error`, а также SQL-типы ещё не согласованы.
...
- [ ] Оформить доменную модель.
- [ ] Определить допустимые состояния первого среза.
...
- Точные Go- и SQL-типы полей Job.
- Формат payload, result и error.
